...
async def _send_batched_notifications(project_id: str, base_url: str):
    ...
    try:
        ...
        for comment_id, reply_id in comment_list:
            ...
            comment_card = f'''<div style="background: #f9fafb; padding: 16px; border-radius: 8px; margin-bottom: 12px;">
                <p style="margin: 0 0 8px 0; font-size: 14px;"><strong>{author}</strong> <span style="color: #888; font-size: 13px;">@ {timecode}</span> <span style="color: #888; font-size: 13px;">· v{version.version_number}{f" ({version.label})" if version.label else ""}</span></p>
                <p style="margin: 0; font-size: 15px; color: #333; white-space: pre-wrap;">{text}</p>
            </div>'''
        ...
        for song_id, song_comments in comments_by_song.items():
            ...
            song_header = f'''<div style="margin: 24px 0 12px 0;">
                <h2 style="margin: 0; font-size: 16px; font-weight: 600; color: #333; border-bottom: 2px solid #6366f1; padding-bottom: 8px;">
                    {song_title} <span style="font-size: 14px; font-weight: 400; color: #888;">({comment_count} {'comment' if comment_count == 1 else 'comments'})</span>
                </h2>
            </div>'''
        ...
        # Email wrapper (light theme like original template)
        body = f'''<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
            <p style="font-size: 18px; font-weight: bold; color: #6366f1; margin: 0 0 8px 0;">{project.title}</p>
            <p style="color: #888; margin: 0 0 16px 0; font-size: 14px;">{total_comments} {'comment' if total_comments == 1 else 'comments'}</p>
